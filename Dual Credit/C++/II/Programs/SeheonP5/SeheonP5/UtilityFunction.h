#include <stdlib.h>
#include <vcclr.h>
#include <string>// from http://blogs.msdn.com/b/slippman/archive/2004/06/02/147090.aspx
...
/*String^ source is a C++/CLI handle to a string object.  The handle
...
If wcstombs successfully converts the multibyte string, it returns 0.
 If the conversion fails,	it returns an error code*/
	...
	errno_t err = wcstombs_s(&i, ch, (size_t)len, wch, (size_t)len);
	if (err == 0)
	{
		target = ch;
	}
